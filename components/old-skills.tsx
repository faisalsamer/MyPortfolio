"use client";

import { useState, useEffect } from "react";
import {
  SiNextdotjs,
  SiReact,
  SiTypescript,
  SiTailwindcss,
  SiNodedotjs,
  SiPhp,
  SiJavascript,
  SiPostgresql,
  SiMysql,
  SiMariadb,
  SiOracle,
  SiFirebase,
  SiPrisma,
  SiSupabase,
  SiGit,
  SiGithub,
  SiFigma,
  SiDocker,
  SiShadcnui,
} from "react-icons/si";
import { TbApi, TbX } from "react-icons/tb";
import Marquee from "react-fast-marquee";
import { Drawer } from "vaul";

// Custom TanStack Icon Component
const TanStackIcon = ({ size = 40 }: { size?: number }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24">
    <path fill="currentColor" d="M11.078.042c.316-.042.65-.014.97-.014c1.181 0 2.341.184 3.472.532a12.3 12.3 0 0 1 3.973 2.086a11.9 11.9 0 0 1 3.432 4.33c1.446 3.15 1.436 6.97-.046 10.107c-.958 2.029-2.495 3.727-4.356 4.965c-1.518 1.01-3.293 1.629-5.1 1.848c-2.298.279-4.784-.129-6.85-1.188c-3.88-1.99-6.518-5.994-6.57-10.382c-.01-.846.003-1.697.17-2.534c.273-1.365.748-2.683 1.463-3.88a12 12 0 0 1 2.966-3.36A12.3 12.3 0 0 1 9.357.3a12 12 0 0 1 1.255-.2l.133-.016zM7.064 19.99c-.535.057-1.098.154-1.557.454c.103.025.222 0 .33 0c.258 0 .52-.01.778.002c.647.028 1.32.131 1.945.303c.8.22 1.505.65 2.275.942c.813.307 1.622.402 2.484.402c.435 0 .866-.001 1.287-.12c-.22-.117-.534-.095-.778-.144a11 11 0 0 1-1.556-.416a12 12 0 0 1-1.093-.467l-.23-.108a15 15 0 0 0-1.012-.44c-.905-.343-1.908-.512-2.873-.408m.808-2.274c-1.059 0-2.13.187-3.083.667q-.346.177-.659.41c-.063.046-.175.106-.199.188s.061.151.11.204c.238-.127.464-.261.718-.357c1.64-.624 3.63-.493 5.268.078c.817.285 1.569.712 2.365 1.046c.89.374 1.798.616 2.753.74c1.127.147 2.412.028 3.442-.48c.362-.179.865-.451 1.018-.847c-.189.017-.36.098-.539.154a9 9 0 0 1-.868.222c-.994.2-2.052.24-3.053.06c-.943-.17-1.82-.513-2.693-.873l-.111-.046l-.223-.092l-.112-.046a26 26 0 0 0-1.35-.527c-.89-.31-1.842-.5-2.784-.5M9.728 1.452c-1.27.28-2.407.826-3.502 1.514c-.637.4-1.245.81-1.796 1.323c-.82.765-1.447 1.695-1.993 2.666c-.563 1-.924 2.166-1.098 3.297c-.172 1.11-.2 2.277-.004 3.388c.245 1.388.712 2.691 1.448 3.897c.248-.116.424-.38.629-.557c.414-.359.85-.691 1.317-.978a3.5 3.5 0 0 1 .539-.264c.07-.029.187-.055.22-.132c.053-.124-.045-.34-.062-.468a7 7 0 0 1-.068-1.109a9.7 9.7 0 0 1 .61-3.177c.29-.76.73-1.45 1.254-2.069c.177-.21.365-.405.56-.6c.115-.114.258-.212.33-.359c-.376 0-.751.108-1.108.218c-.769.237-1.518.588-2.155 1.084c-.291.226-.504.522-.779.76c-.084.073-.235.17-.352.116c-.176-.083-.149-.43-.169-.59c-.078-.612.154-1.387.45-1.918c.473-.852 1.348-1.58 2.376-1.555c.444.011.833.166 1.257.266c-.107-.153-.252-.264-.389-.39a5.4 5.4 0 0 0-1.107-.8c-.163-.085-.338-.136-.509-.2c-.086-.03-.195-.074-.227-.17c-.06-.177.26-.342.377-.417c.453-.289 1.01-.527 1.556-.54c.854-.021 1.688.452 2.04 1.258c.123.284.16.583.184.885l.004.057l.006.085l.002.029l.005.057l.004.056c.268-.218.457-.54.718-.774c.612-.547 1.45-.79 2.245-.544a2.97 2.97 0 0 1 1.71 1.378c.097.173.365.595.171.767c-.152.134-.344.03-.504-.026a3 3 0 0 0-.372-.094l-.068-.014l-.069-.013a3.9 3.9 0 0 0-1.377-.002c-.282.05-.557.15-.838.192v.06c.768.006 1.51.444 1.89 1.109c.157.275.235.59.295.9c.075.38.022.796-.082 1.168c-.035.125-.098.336-.247.365c-.106.02-.195-.085-.256-.155a4.6 4.6 0 0 0-.492-.522a20 20 0 0 0-1.467-1.14c-.267-.19-.56-.44-.868-.556c.087.208.171.402.2.63c.088.667-.192 1.296-.612 1.798a2.6 2.6 0 0 1-.426.427c-.067.05-.151.114-.24.1c-.277-.044-.31-.463-.353-.677c-.144-.726-.086-1.447.114-2.158c-.178.09-.307.287-.418.45a5.3 5.3 0 0 0-.612 1.138c-.61 1.617-.604 3.51.186 5.066c.088.174.221.15.395.15h.157a3 3 0 0 1 .472.018c.08.01.193 0 .257.06c.077.072.036.194.018.282c-.05.246-.066.469-.066.72c.328-.051.419-.576.535-.84c.131-.298.265-.597.387-.9c.06-.148.14-.314.119-.479c-.024-.185-.157-.381-.25-.54c-.177-.298-.378-.606-.508-.929c-.104-.258-.007-.58.286-.672c.161-.05.334.049.439.166c.22.244.363.609.523.896l1.249 2.248q.159.286.32.57c.043.074.086.188.173.219c.077.028.182-.012.26-.027c.198-.04.398-.083.598-.12c.24-.043.605-.035.778-.222c-.253-.08-.545-.075-.808-.057c-.158.01-.333.067-.479-.025c-.216-.137-.36-.455-.492-.667c-.326-.525-.633-1.057-.945-1.59l-.05-.084l-.1-.17q-.075-.126-.149-.255c-.037-.066-.092-.153-.039-.227c.056-.076.179-.08.29-.081h.021q.066.001.117-.004a10 10 0 0 1 1.347-.107c-.035-.122-.135-.26-.103-.39c.071-.292.49-.383.686-.174c.131.14.207.334.292.504c.113.223.24.44.361.66c.211.383.441.757.658 1.138l.055.094l.028.047c.093.156.187.314.238.489c-.753-.035-1.318-.909-1.646-1.499c-.027.095.016.179.05.27q.103.282.262.54c.152.244.326.495.556.673c.408.315.945.317 1.436.283c.315-.022.708-.165 1.018-.068s.434.438.25.7c-.138.196-.321.27-.55.3c.162.346.373.667.527 1.02c.064.146.13.37.283.448c.102.051.248.003.358 0c-.11-.292-.317-.54-.419-.839c.31.015.61.176.898.28c.567.202 1.128.424 1.687.648l.258.104c.23.092.462.183.689.283c.083.037.198.123.29.07c.074-.043.123-.146.169-.215a10.3 10.3 0 0 0 1.393-3.208c.75-2.989.106-6.287-1.695-8.783c-.692-.96-1.562-1.789-2.522-2.476c-2.401-1.718-5.551-2.407-8.44-1.768m4.908 14.904c-.636.166-1.292.317-1.945.401c.086.293.296.577.45.84c.059.101.122.237.24.281c.132.05.292-.03.417-.072c-.058-.158-.155-.3-.235-.45c-.033-.06-.084-.133-.056-.206c.05-.137.263-.13.381-.153c.31-.063.617-.142.928-.204c.114-.023.274-.085.389-.047c.086.03.138.1.187.174l.022.033q.043.07.097.122c.125.113.313.13.472.162c-.097-.219-.259-.41-.362-.63c-.06-.127-.11-.315-.242-.388c-.182-.102-.557.089-.743.137m-4.01-1.457c-.03.38-.147.689-.33 1.019c.21.026.423.036.629.087c.154.038.296.11.449.153c-.082-.224-.233-.423-.35-.63c-.12-.208-.226-.462-.398-.63"/>
  </svg>
);

// All skills data
const allSkills = [
  { name: "Next.js", icon: SiNextdotjs, color: "#000000", category: "Frontend" },
  { name: "React", icon: SiReact, color: "#61DAFB", category: "Frontend" },
  { name: "TypeScript", icon: SiTypescript, color: "#3178C6", category: "Frontend" },
  { name: "Tailwind CSS", icon: SiTailwindcss, color: "#06B6D4", category: "Frontend" },
  { name: "TanStack Query", icon: TanStackIcon, color: "#FF4154", category: "Frontend" },
  { name: "TanStack Router", icon: TanStackIcon, color: "#FF4154", category: "Frontend" },
  { name: "shadcn/ui", icon: SiShadcnui, color: "#000000", category: "Frontend" },
  { name: "Node.js", icon: SiNodedotjs, color: "#339933", category: "Backend" },
  { name: "PHP", icon: SiPhp, color: "#777BB4", category: "Backend" },
  { name: "JavaScript", icon: SiJavascript, color: "#F7DF1E", category: "Backend" },
  { name: "REST API", icon: TbApi, color: "#009688", category: "Backend" },
  { name: "PostgreSQL", icon: SiPostgresql, color: "#4169E1", category: "Database" },
  { name: "MySQL", icon: SiMysql, color: "#4479A1", category: "Database" },
  { name: "MariaDB", icon: SiMariadb, color: "#003545", category: "Database" },
  { name: "Oracle DB", icon: SiOracle, color: "#F80000", category: "Database" },
  { name: "Firebase", icon: SiFirebase, color: "#FFCA28", category: "Cloud" },
  { name: "Firestore", icon: SiFirebase, color: "#FFCA28", category: "Database" },
  { name: "Prisma", icon: SiPrisma, color: "#2D3748", category: "Database" },
  { name: "Supabase", icon: SiSupabase, color: "#3ECF8E", category: "Cloud" },
  { name: "Git", icon: SiGit, color: "#F05032", category: "Tools" },
  { name: "GitHub", icon: SiGithub, color: "#181717", category: "Tools" },
  { name: "Figma", icon: SiFigma, color: "#F24E1E", category: "Tools" },
  { name: "Docker", icon: SiDocker, color: "#2496ED", category: "Tools" },
];

// Major skills for marquee (top 8-10 most important)
const majorSkills = [
  { name: "Next.js", icon: SiNextdotjs, color: "#000000" },
  { name: "React", icon: SiReact, color: "#61DAFB" },
  { name: "TypeScript", icon: SiTypescript, color: "#3178C6" },
  { name: "Tailwind CSS", icon: SiTailwindcss, color: "#06B6D4" },
  { name: "Node.js", icon: SiNodedotjs, color: "#339933" },
  { name: "PostgreSQL", icon: SiPostgresql, color: "#4169E1" },
  { name: "Supabase", icon: SiSupabase, color: "#3ECF8E" },
  { name: "Docker", icon: SiDocker, color: "#2496ED" },
];

// Preview icons for the View All button (first 4 diverse skills)
const previewSkills = [
  { name: "React", icon: SiReact, color: "#61DAFB" },
  { name: "Node.js", icon: SiNodedotjs, color: "#339933" },
  { name: "PostgreSQL", icon: SiPostgresql, color: "#4169E1" },
  { name: "Docker", icon: SiDocker, color: "#2496ED" },
];

// Skills Content Component (reused for both drawer and dialog)
const SkillsContent = () => (
  <div className="p-6">
    {["Frontend", "Backend", "Database", "Cloud", "Tools"].map((category) => {
      const categorySkills = allSkills.filter((s) => s.category === category);
      if (categorySkills.length === 0) return null;

      return (
        <div key={category} className="mb-8 last:mb-0">
          {/* Category Header */}
          <div className="flex items-center gap-3 mb-4">
            <div
              className="h-1 w-10 rounded-full"
              style={{
                background: "linear-gradient(90deg, var(--color-primary-purple), var(--color-primary-blue))",
              }}
            />
            <h4
              className="text-xl font-semibold"
              style={{
                color: "var(--ui-heading-color)",
                fontFamily: "var(--font-family-secondary)",
              }}
            >
              {category}
            </h4>
          </div>

          {/* Skills Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {categorySkills.map((skill) => {
              const IconComponent = skill.icon;
              return (
                <div
                  key={skill.name}
                  className="group relative p-4 rounded-lg border transition-all duration-300 hover:scale-105 hover:shadow-lg"
                  style={{
                    backgroundColor: "var(--color-layout-background)",
                    borderColor: "var(--color-gray-200)",
                  }}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className="transition-all duration-300 group-hover:scale-110 shrink-0"
                      style={{ color: skill.color }}
                    >
                      {IconComponent === TanStackIcon ? (
                        <TanStackIcon size={28} />
                      ) : (
                        <IconComponent size={28} />
                      )}
                    </div>
                    <h5
                      className="text-sm font-medium leading-tight"
                      style={{ color: "var(--ui-heading-color)" }}
                    >
                      {skill.name}
                    </h5>
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      );
    })}
  </div>
);

export default function Skills() {
  const [isOpen, setIsOpen] = useState(false);
  const [isMobile, setIsMobile] = useState(false);

  useEffect(() => {
    const checkMobile = () => setIsMobile(window.innerWidth < 768);
    checkMobile();
    window.addEventListener("resize", checkMobile);
    return () => window.removeEventListener("resize", checkMobile);
  }, []);

  return (
    <>
      <section
        id="skills"
        className="py-12 md:py-16"
        style={{ backgroundColor: "var(--color-layout-background)" }}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
          {/* Section Header */}
          <div className="text-center mb-8 md:mb-10">
            <h2
              className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 scroll-animate fade-up"
              style={{
                color: "var(--ui-heading-color)",
                fontFamily: "var(--font-family-secondary)",
              }}
            >
              Technical Stack
            </h2>
            <p
              className="text-lg md:text-xl scroll-animate fade-up delay-100"
              style={{ color: "var(--ui-subheading-color)" }}
            >
              Technologies I use to build exceptional digital experiences
            </p>
          </div>

          {/* Marquee Container */}
          <div
            className="relative rounded-2xl border p-6 md:p-8 scroll-animate scale-in"
            style={{
              backgroundColor: "var(--color-card)",
              borderColor: "var(--color-gray-200)",
              boxShadow: "var(--ui-shadow-modern)",
            }}
          >


            {/* Marquee */}
            <div className="relative">
              <Marquee gradient={false} speed={25} pauseOnHover={true}>
                {majorSkills.map((tech, index) => {
                  const IconComponent = tech.icon;
                  return (
                    <div
                      key={`${tech.name}-${index}`}
                      className="group relative mx-2.5 px-5 py-3.5 rounded-lg border transition-all duration-300 hover:scale-105 hover:shadow-lg"
                      style={{
                        backgroundColor: "var(--color-layout-background)",
                        borderColor: "var(--color-gray-200)",
                      }}
                    >
                      <div className="relative flex items-center gap-3">
                        <div
                          className="transition-all duration-300 group-hover:scale-110 shrink-0"
                          style={{ color: tech.color }}
                        >
                          {IconComponent === TanStackIcon ? (
                            <TanStackIcon size={32} />
                          ) : (
                            <IconComponent size={32} />
                          )}
                        </div>
                        <h4
                          className="text-sm md:text-base font-medium leading-tight whitespace-nowrap"
                          style={{ color: "var(--ui-heading-color)" }}
                        >
                          {tech.name}
                        </h4>
                      </div>
                    </div>
                  );
                })}
              </Marquee>
            </div>

            {/* View All Button */}
            <div className="mt-6 flex items-center justify-center gap-3">
              {/* Preview Icons Stack */}
              <div className="flex -space-x-3">
                {previewSkills.map((skill, index) => {
                  const IconComponent = skill.icon;
                  return (
                    <div
                      key={skill.name}
                      className="w-10 h-10 rounded-full border flex items-center justify-center border-neutral-200/70!"
                      style={{
                        backgroundColor: "var(--color-card)",
                        borderColor: "var(--color-layout-background)",
                        color: skill.color,
                        zIndex: previewSkills.length - index,
                      }}
                    >
                      <IconComponent size={20} />
                    </div>
                  );
                })}
                {/* Count badge */}
                <div
                  className="w-10 h-10 rounded-full border-2 flex items-center justify-center text-xs font-bold"
                  style={{
                    backgroundColor: "var(--color-card)",
                    borderColor: "var(--color-layout-background)",
                    color: "var(--ui-heading-color)",
                    zIndex: 0,
                  }}
                >
                  +{allSkills.length - previewSkills.length}
                </div>
              </div>

              {/* View All Button */}
              <button
                onClick={() => setIsOpen(true)}
                className="px-6 py-2.5 rounded-lg font-medium text-sm transition-all duration-300 hover:scale-105 hover:shadow-lg"
                style={{
                  background: "linear-gradient(135deg, var(--color-primary-purple), var(--color-primary-blue))",
                  color: "white",
                }}
              >
                View All Skills
              </button>
            </div>
          </div>
        </div>
      </section>

      {/* Mobile: Drawer (slides up from bottom) */}
      {isMobile ? (
        <Drawer.Root open={isOpen} onOpenChange={setIsOpen}>
          <Drawer.Portal>
            <Drawer.Overlay className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50" />
            <Drawer.Content
              className="fixed inset-0 z-50 flex flex-col rounded-t-3xl"
              style={{
                backgroundColor: "var(--color-card)",
              }}
            >
              {/* Drag Handle */}
              <div className="flex justify-center py-3">
                <div
                  className="w-12 h-1.5 rounded-full"
                  style={{ backgroundColor: "var(--color-gray-300)" }}
                />
              </div>

              {/* Header */}
              <div
                className="px-6 pb-4 border-b flex items-center justify-between"
                style={{ borderColor: "var(--color-gray-200)" }}
              >
                <Drawer.Title
                  className="text-2xl font-bold"
                  style={{
                    color: "var(--ui-heading-color)",
                    fontFamily: "var(--font-family-secondary)",
                  }}
                >
                  All Skills ({allSkills.length})
                </Drawer.Title>
                <button
                  onClick={() => setIsOpen(false)}
                  className="p-2 rounded-lg"
                  style={{ color: "var(--ui-heading-color)" }}
                >
                  <TbX size={24} />
                </button>
              </div>

              {/* Content */}
              <div className="overflow-y-auto flex-1 custom-scrollbar">
                <SkillsContent />
              </div>
            </Drawer.Content>
          </Drawer.Portal>
        </Drawer.Root>
      ) : (
        /* Desktop: Dialog */
        isOpen && (
          <div
            className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm"
            onClick={() => setIsOpen(false)}
          >
            <div
              className="relative w-full max-w-4xl max-h-[90vh] rounded-2xl border overflow-hidden"
              style={{
                backgroundColor: "var(--color-card)",
                borderColor: "var(--color-gray-200)",
                boxShadow: "var(--ui-shadow-modern)",
              }}
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div
                className="sticky top-0 z-10 px-6 py-4 border-b flex items-center justify-between"
                style={{
                  backgroundColor: "var(--color-card)",
                  borderColor: "var(--color-gray-200)",
                }}
              >
                <h3
                  className="text-2xl font-bold"
                  style={{
                    color: "var(--ui-heading-color)",
                    fontFamily: "var(--font-family-secondary)",
                  }}
                >
                  All Skills ({allSkills.length})
                </h3>
                <button
                  onClick={() => setIsOpen(false)}
                  className="p-2 rounded-lg transition-colors"
                  style={{ color: "var(--ui-heading-color)" }}
                >
                  <TbX size={24} />
                </button>
              </div>

              {/* Content */}
              <div className="overflow-y-auto max-h-[calc(90vh-80px)] custom-scrollbar">
                <SkillsContent />
              </div>
            </div>
          </div>
        )
      )}
    </>
  );
}
